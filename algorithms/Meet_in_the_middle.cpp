...
void solve()
{
    ...
    auto get_subset_sums = [&](ll l, ll r) -> vector<ll> {
		ll len = r - l + 1;
		vector<ll> res;
    ...
    ll ans = 0;
	for (ll i : left) {
		auto low_iterator = lower_bound(right.begin(), right.end(), m - i);
		auto high_iterator = upper_bound(right.begin(), right.end(), m - i);
